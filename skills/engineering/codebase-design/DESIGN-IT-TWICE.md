...
Call `run_subagent` 3+ times in one block, each with profile `mattpocock-skills:researcher` and `is_background: true`. Each must produce a **radically different** interface for the deepened module.
